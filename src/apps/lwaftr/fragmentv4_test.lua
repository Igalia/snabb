...
function test_reassemble_unneeded()
   ...
   local code, r = fragmentv4.reassemble_ipv4({ pkt })
...
function test_reassemble_two_missing_fragments()
   ...
   local pkt = assert(make_ipv4_packet(1200))
   local code, fragments = fragmentv4.fragment_ipv4(pkt, 1000)
   assert(code == fragmentv4.FRAGMENT_OK)
   assert(#fragments == 2)

   assert(fragmentv4.REASSEMBLE_MISSING_FRAGMENT ==
          (fragmentv4.reassemble_ipv4({ fragments[1] })))
   assert(fragmentv4.REASSEMBLE_MISSING_FRAGMENT ==
          (fragmentv4.reassemble_ipv4({ fragments[2] })))
end
...
function test_reassemble_three_missing_fragments()
   ...
   local code, fragments = fragmentv4.fragment_ipv4(pkt, 400)
   assert(code == fragmentv4.FRAGMENT_OK)
   assert(#fragments == 3)
   ...
   assert(fragmentv4.REASSEMBLE_MISSING_FRAGMENT ==
          (fragmentv4.reassemble_ipv4({ fragments[2] })))
   assert(fragmentv4.REASSEMBLE_MISSING_FRAGMENT ==
          (fragmentv4.reassemble_ipv4({ fragments[3] })))

   assert(fragmentv4.REASSEMBLE_MISSING_FRAGMENT ==
          (fragmentv4.reassemble_ipv4({ fragments[1], fragments[2] })))
   assert(fragmentv4.REASSEMBLE_MISSING_FRAGMENT ==
          (fragmentv4.reassemble_ipv4({ fragments[1], fragments[3] })))
   assert(fragmentv4.REASSEMBLE_MISSING_FRAGMENT ==
          (fragmentv4.reassemble_ipv4({ fragments[3], fragments[3] })))

   assert(fragmentv4.REASSEMBLE_MISSING_FRAGMENT ==
          (fragmentv4.reassemble_ipv4({ fragments[2], fragments[1] })))
   assert(fragmentv4.REASSEMBLE_MISSING_FRAGMENT ==
          (fragmentv4.reassemble_ipv4({ fragments[3], fragments[1] })))
   assert(fragmentv4.REASSEMBLE_MISSING_FRAGMENT ==
          (fragmentv4.reassemble_ipv4({ fragments[2], fragments[3] })))
end
...
function test_reassemble_two()
   ...
   local function try(f)
	   local code, pkt = fragmentv4.reassemble_ipv4(f)
